        ...
        for (const d of dirs) {
            if (d.startsWith('chromium-')) {
                const p = process.platform === 'win32' 
                    ...
                    ? path.join(dir, d, 'Chromium.app', 'Contents', 'MacOS', 'Chromium')
...
function sendLog(log, percent = null) {
    if (mainWindowRef && !mainWindowRef.isDestroyed()) {
        mainWindowRef.webContents.send('browser-runtime-progress', {
            browserName: 'cloakbrowser', log, percent
        });
    }
}
...
async function checkStatus() {
    ...
    if (downloaded && exePath) {
        try {
            ...
            const baseDir = path.dirname(process.platform === 'darwin' ? path.dirname(path.dirname(path.dirname(exePath))) : path.dirname(exePath)); 
...
async function install() {
    if (activeInstall.running) return { success: false, error: 'CloakBrowser is already installing.' };
